...
public class StatusBarUtil {
    ...
    public static void setColor(Activity activity, int color, int statusBarAlpha) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            if (count > 0 && decorView.getChildAt(count - 1) instanceof StatusBarView) {
                Logger.i(TAG,"setcolor==");
                decorView.getChildAt(count - 1).setBackgroundColor(calculateStatusColor(color, statusBarAlpha));
            } else {
                Logger.i(TAG,"setcolor==addview");
     ...
     * @param isHide  布尔值
     */
    public static void isHideStatusBar(Activity activity,boolean isHide) {
        if (isHide) {
            ...
            if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.KITKAT) {
                ...
                Logger.i(TAG, "child" + childCount);
                for (int i=0;i<childCount;i++){
                    if (childCount > 0 || decorView.getChildAt(i) instanceof StatusBarView) {
            ...
            if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.KITKAT) {
                ...
                if (childCount > 0 || decorView.getChildAt(childCount - 1) instanceof StatusBarView) {
                    decorView.getChildAt(childCount - 1).setBackgroundColor(calculateStatusColor(BaseApplication.getInstance().getResources().getColor(R.color.gray), 0));
                }else{
                    decorView.addView(createStatusBarView(activity, BaseApplication.getInstance().getResources().getColor(R.color.gray), 0));
                }
                for (int i = 0; i < childCount; i++) {
                    if (decorView.getChildAt(i) instanceof StatusBarView) {
                        Logger.i(TAG, "chilecountName" + decorView.getChildAt(i).toString()
                                +"child"+i);
